                                ...
                                esp_lcd_panel_io_handle_t *panel_io_handle)
{
    ...
    ESP_LOGI(TAG, "Virtual display configured: %dx%d (no physical display)", 
    ...
    *panel_io_handle = NULL;
    ...
    ESP_LOGI(TAG, "ESP BSP DevKit initialized (virtual display: %dx%d)", 
             config->width, config->height);
...
{
    ESP_LOGD(TAG, "%s virtual display (no physical display on DevKit)", 
